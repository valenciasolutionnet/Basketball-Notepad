// Offline support: ballfields often have no signal. The app shell and built
// assets are cached; live-game sync (/api/) always goes to the network.
const CACHE = "baseball-notepad-v1";
...
self.addEventListener("push", (event) => {
  let data = { title: "Baseball Notepad", body: "" };
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || "Baseball Notepad", {
      body: data.body,
      icon: "/favicon.svg",
      badge: "/favicon.svg",
      tag: "baseball-notepad-admin",
    }),
  );
});
